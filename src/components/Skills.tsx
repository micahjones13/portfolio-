  ...
  return (
    <section className="w-full space-y-6">
      ...
      <div className="w-full max-w-6xl overflow-hidden py-8 mx-auto px-4 skills-scroll">
        <div ref={scrollContainerRef} className="skills-scroll-container bg-slate-800/50 py-4">
          <div ref={scrollContentRef} className="skills-scroll-content flex gap-8">
            {[...Array(4)].map((_, setIndex) => (
              SKILLS.map((skill, index) => (
                <div 
                  key={`${skill.name}-${setIndex}-${index}`}
                  className="group relative inline-flex shrink-0 items-center justify-center bg-slate-700 px-6 py-3 rounded-lg text-gray-100 font-medium shadow-lg hover:bg-slate-600 transition-colors"
                >
                  {skill.name}
                  {skill.description && (
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-normal pointer-events-none w-48 text-center shadow-xl z-[100] border border-sky-300">
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 border-8 border-transparent border-b-sky-300"></div>
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 border-[7px] border-transparent border-b-gray-900"></div>
                      {skill.description}
                    </div>
                  )}
                </div>
